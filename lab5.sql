...
--1--
go
CREATE FUNCTION C1 (@masp VARCHAR(50))
RETURNS VARCHAR(50)
AS
BEGIN
...
--2--
go
CREATE FUNCTION C2(@x INT, @y INT)
RETURNS MONEY
AS
BEGIN
...
go
go
SELECT dbo.C2(2016,2019)
go


--3--
go 
CREATE FUNCTION C3(@tenSanPham nvarchar(50), @nam int)
RETURNS int
AS
BEGIN
...
go
go
SELECT dbo.C3('Galaxy Note 11',2020)
go

--4--
go
CREATE FUNCTION dbo.C4(
    @ngay_bat_dau DATE,
    @ngay_ket_thuc DATE
)
RETURNS MONEY
...
go
go
SELECT dbo.C4('2018-01-01', '2020-12-31') AS TongGiaTriNhap;
go

--5--
go
CREATE FUNCTION C5(@tenHang NVARCHAR(20), @nam INT)
RETURNS MONEY
AS
BEGIN
...
go
go
SELECT dbo.C5(N'Samsung', 2020) AS 'TongGiaTriXuat';
go

--6--
go
CREATE FUNCTION C6 (@tenPhong NVARCHAR(30))
RETURNS TABLE
AS
RETURN
...
go
go
SELECT * FROM C6(N'Vật tư')
go

--7--
go
CREATE FUNCTION C7(@ten_sp NVARCHAR(20), @ngay_xuat DATE)
RETURNS INT
AS
BEGIN
...
go
go
SELECT dbo.C7('Samsung', '12-12-2020')
go

--8--
go
CREATE FUNCTION C8 (@InvoiceNumber NCHAR(10))
RETURNS NVARCHAR(20)
AS
BEGIN
...
--9--
go
CREATE FUNCTION C9(@tenSP NVARCHAR(20), @nam INT)
RETURNS INT
AS
BEGIN
...
go
go
SELECT dbo.C9('Galaxy V21', 2020) AS TongNhapXuat;
go

--10--
go
CREATE FUNCTION C10(@tenhang NVARCHAR(20))
RETURNS INT
AS
BEGIN
    DECLARE @soluong INT;
...
go
go
SELECT dbo.C10('Samsung') AS 'Tổng số lượng sản phẩm của hãng'
go